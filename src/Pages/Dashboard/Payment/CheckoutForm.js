...
const CheckoutForm = ({ order }) => {
    ...
    useEffect(() => {
        ...
        fetch('https://thawing-wildwood-78264.herokuapp.com/create-payment-intent', {
            method: 'POST',
            headers: {
    ...
    const handleSubmit = async (event) => {
        ...
        if (intentError) {
            ...
            fetch(`https://thawing-wildwood-78264.herokuapp.com/order/${_id}`, {
                method: 'PATCH',
                headers: {
                    'content-type': 'application/json',
                    'authorization': `Bearer ${localStorage.getItem('accessToken')}`
    ...
    return (
        <>
            <form onSubmit={handleSubmit}>
                ...
                />
                <button className='btn btn-success btn-sm text-white font-bold mt-4' type="submit" disabled={!stripe || !clientSecret || success}>
                    Pay
                </button>
            </form>
            {
